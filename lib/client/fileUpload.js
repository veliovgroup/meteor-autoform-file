...
Template.afFileUpload.helpers({
  ...
  uploadedFile() {
    ...
    const file = template.collection.findOne({_id:_id});
    if (!file) return null;
    file.hasBootstrap = template.hasBootstrap();
    return file;
  },
  ...
  hasBootstrap(){
    return Template.instance().hasBootstrap();
  }
});
  ...
  'change [data-files-collection-upload]'(e, template) {
    if (e.currentTarget.files && e.currentTarget.files[0]) {
      ...
      try {
        ...
        ctx = {
          reset(){},
		  addValidationErrors(err){
            console.warn("attempt to add validation errors", err);
            window.alertbox("error", err[0].value);
          }
        }
      }


      // enable hasBootstrap in upload
      // context, in case, that a custom
      // template want to use it
      this.hasBootstrap = template.hasBootstrap();
